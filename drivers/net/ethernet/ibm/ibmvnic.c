...
static void ibmvnic_release_sub_crqs(struct ibmvnic_adapter *);
...
static void set_adapter_status(struct ibmvnic_adapter *adapter, u32 status)
{
	u32 mask = 0x0000ffff;

	switch (status) {
	case VNIC_PROBING:
		adapter->status = VNIC_PROBING;
		break;
	case VNIC_PROBED:
		clear_adapter_status(adapter, VNIC_PROBING);
		adapter->status = VNIC_PROBED | (adapter->status & mask);
		break;
	case VNIC_OPENING:
		adapter->status |= VNIC_OPENING;
		break;
	case VNIC_OPEN:
		clear_adapter_status(adapter, VNIC_OPENING);
		adapter->status = VNIC_OPEN | (adapter->status & mask);
		break;
	case VNIC_CLOSING:
		adapter->status |= VNIC_CLOSING;
		break;
	case VNIC_CLOSED:
		clear_adapter_status(adapter, VNIC_CLOSING);
		adapter->status = VNIC_CLOSED | (adapter->status & mask);
		break;
	case VNIC_REMOVING:
		adapter->status |= VNIC_REMOVING;
		break;
	case VNIC_REMOVED:
		clear_adapter_status(adapter, VNIC_REMOVING);
		adapter->status = VNIC_REMOVED | (adapter->status & mask);
		break;
	case VNIC_RESETTING:
		adapter->status |= VNIC_RESETTING;
		break;
	}
}
...
static int ibmvnic_init_rx_pools(struct net_device *netdev)
{
...
static void ibmvnic_release_tx_pools(struct ibmvnic_adapter *adapter)
{
...
static int ibmvnic_init_tx_pools(struct net_device *netdev)
{
	struct ibmvnic_adapter *adapter = netdev_priv(netdev);
	struct device *dev = &adapter->vdev->dev;
	struct ibmvnic_tx_pool *tx_pool;
...
static int ibmvnic_login(struct net_device *netdev)
{
	...
	netdev_err(netdev, "in login\n");
	do {
		if (adapter->renegotiate) {
			netdev_err(netdev, "renegotiate\n");
			adapter->renegotiate = false;
			ibmvnic_release_sub_crqs(adapter);

			reinit_completion(&adapter->init_done);
			netdev_err(netdev, "sending cap query\n");
		...
		reinit_completion(&adapter->init_done);
		netdev_err(netdev, "sending login\n");
		send_login(adapter);
		if (!wait_for_completion_timeout(&adapter->init_done,
						 timeout)) {
...
static int ibmvnic_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	if (new_mtu > adapter->req_mtu - ETH_HLEN ||
		       ...
		       enum ibmvnic_reset_reason reason)
{
	...
	if (!rwi) {
		netdev_err(netdev, "Driver reset failed\n");
		ibmvnic_close(netdev);
		return;
	}

	netdev_err(netdev, "Adding reset work\n");
	rwi->reset_reason = reason;
	list_add_tail(&rwi->list, &adapter->reset_work_items);
...
static void ibmvnic_tx_timeout(struct net_device *dev)
{
	...
	vnic_reset(adapter, VNIC_RESET_TIMEOUT);
}
...
restart_poll:
	while (frames_processed < budget) {
		...
		/* do error checking */
		if (next->rx_comp.rc) {
			if (!(adapter->status & (VNIC_CLOSED | VNIC_CLOSING)))
...
static void __release_scrq(struct ibmvnic_adapter *adapter,
			   struct ibmvnic_sub_crq_queue *scrq)
{
	if (!scrq)
		return;
	...
	release_sub_crq_queue(adapter, scrq);
}

static void ibmvnic_release_sub_crqs(struct ibmvnic_adapter *adapter)
{
	int i;

	if (adapter->tx_scrq) {
		for (i = 0; i < adapter->req_tx_queues; i++) {
			__release_scrq(adapter, adapter->tx_scrq[i]);
			adapter->tx_scrq[i] = NULL;
		}
	...
	if (adapter->rx_scrq) {
		for (i = 0; i < adapter->req_rx_queues; i++) {
			__release_scrq(adapter, adapter->rx_scrq[i]);
			adapter->rx_scrq[i] = NULL;
		}
...
static void ibmvnic_init_sub_crqs(struct ibmvnic_adapter *adapter, int retry)
{
	struct device *dev = &adapter->vdev->dev;
	...
	if (!retry) {
		...
		pr_err("tx queues = %d rx queues = %d\n", adapter->req_tx_queues, adapter->req_rx_queues);
				   ...
				   struct ibmvnic_adapter *adapter)
{
	...
	case PARTIALSUCCESS:
		...
		*req_value = be64_to_cpu(crq->request_capability_rsp.number);
		ibmvnic_init_sub_crqs(adapter, 1);
		return;
	default:
...
out:
	if (atomic_read(&adapter->running_cap_crqs) == 0) {
		adapter->wait_capability = false;
		ibmvnic_init_sub_crqs(adapter, 0);
			       ...
			       struct ibmvnic_adapter *adapter)
{
	...
	case IBMVNIC_CRQ_XPORT_EVENT:
		if (gen_crq->cmd == IBMVNIC_PARTITION_MIGRATED) {
			...
			vnic_reset(adapter, VNIC_RESET_MOBILITY);
...
static void ibmvnic_release_crq_queue(struct ibmvnic_adapter *adapter)
{
	...
	netdev_err(adapter->netdev, "Releasing CRQ\n");
...
static int ibmvnic_init_crq_queue(struct ibmvnic_adapter *adapter)
{
	...
	dev_err(dev, "msgs allocated\n");
	if (!crq->msgs)
		return -ENOMEM;

	dev_err(dev, "mapping msgs\n");
	...
	netdev_err(adapter->netdev, "registering irq 0x%x\n", vdev->irq);
...
static int ibmvnic_probe(struct vio_dev *dev, const struct vio_device_id *id)
{
	...
	dev_err(&dev->dev, "entering ibmvnic_probe for UA 0x%x\n",
		dev->unit_address);
	...
	dev_err(&dev->dev, "alloc_ethdev\n");
	netdev = alloc_etherdev_mq(sizeof(struct ibmvnic_adapter),
				   IBMVNIC_MAX_TX_QUEUES);
	if (!netdev)
		return -ENOMEM;

	dev_err(&dev->dev, "set netpriv\n");
	adapter = netdev_priv(netdev);
	memset(adapter, 0, sizeof(*adapter));
	set_adapter_status(adapter, VNIC_PROBING);
	dev_set_drvdata(&dev->dev, netdev);
	adapter->vdev = dev;
	adapter->netdev = netdev;

	dev_err(&dev->dev, "addr_copy\n");
	...
	dev_err(&dev->dev, "register netdev\n");
...
static int ibmvnic_remove(struct vio_dev *dev)
{
	...
	netdev_err(netdev, "in remove, releasing resources\n");
	ibmvnic_release_resources(adapter);
	ibmvnic_release_sub_crqs(adapter);
	ibmvnic_release_crq_queue(adapter);

	set_adapter_status(adapter, VNIC_REMOVED);
	mutex_unlock(&adapter->reset_lock);

	netdev_err(netdev, "in remove, free'ing netdev\n");
	free_netdev(netdev);
